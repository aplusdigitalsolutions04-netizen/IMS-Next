            ...
            <div className="space-y-1">
              ...
              {expandedMenus.master && (
                <div className="space-y-1 ml-4 border-l border-slate-100 animate-in slide-in-from-top-1">
                  ...
                  {hasPermission('clientMaster') && (
                    <Link href="/clientMaster" className={`w-full flex items-center gap-3 pl-4 pr-3 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'clientMaster' ? 'bg-indigo-50 text-indigo-600' : 'text-slate-600 hover:bg-slate-100'}`}>
                      <UsersIcon size={14} className="flex-shrink-0" /> <span className="truncate">Client Master</span>
                    </Link>
                  )}
                  {hasPermission('carePackMaster') && (
                    <Link href="/carePackMaster" className={`w-full flex items-center gap-3 pl-4 pr-3 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'carePackMaster' ? 'bg-indigo-50 text-indigo-600' : 'text-slate-600 hover:bg-slate-100'}`}>
                      <ShieldCheck size={14} className="flex-shrink-0" /> <span className="truncate">Care Pack</span>
                    </Link>
                  )}
                </div>
              )}
