...
export default function OrderDetailModal({
  ...
  const [currentEmailOrderGuid, setCurrentEmailOrderGuid] = React.useState(null);
  ...
  const loadEmailDraft = async (orderGuid, templateGuid) => {
  ...
  const openEmailCompose = async () => {
    const orderGuid = selectedBatch?.items?.[0]?.orderGuid || selectedBatch?.items?.[0]?._orderId;
    if (!orderGuid) return;
    setCurrentEmailOrderGuid(orderGuid);
  ...
  const confirmTemplate = async () => {
    ...
    await loadEmailDraft(currentEmailOrderGuid, pickedTemplateGuid);
  };
                  ...
                  <div className="p-4 space-y-3 max-h-[72vh] overflow-y-auto">
                    {isEditMode ? (
                      ...
                      <>
                        ...
                        <div className="flex items-center gap-1.5 bg-slate-100 p-1 rounded-xl w-fit">
                          {[
                            ...
                            <button
                              key={tab.id}
                              onClick={() => {
                                setModalDetailTab(tab.id);
                                if (tab.id === "email" && !emailDraft && !templatePicker) openEmailCompose();
                              }}
                        ...
                        {/* ── EMAIL TAB ── */}
                        {modalDetailTab === "email" && <div className="space-y-3">
                          {templatePicker && (
