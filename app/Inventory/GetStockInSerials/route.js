...
export const GET = withErrorHandling(async (request) => {
  ...
  const [rows] = await mysqlPool.query("SELECT serialId, serialNumber, carePack, carePackPrice FROM inventorystockinserial WHERE stockInDetailId = ? AND isDeleted = 0 AND companyGuid = ?", [detailId, user.companyId]);
