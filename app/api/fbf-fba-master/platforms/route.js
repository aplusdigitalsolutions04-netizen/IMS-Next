...
export const GET = withErrorHandling(async (request) => {
  ...
  const [rows] = await mysqlPool.query("SELECT * FROM fbf_fba_platforms WHERE companyGuid = ? ORDER BY name ASC", [user.companyId]);
...
export const POST = withErrorHandling(async (request) => {
  ...
  await mysqlPool.query("INSERT INTO fbf_fba_platforms (guid, name, companyGuid) VALUES (?, ?, ?)", [newGuid, name.trim(), user.companyId]);
