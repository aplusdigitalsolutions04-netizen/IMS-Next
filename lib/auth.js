...
// Same "view access to a non-protected tab is enough to do its work" rule as
// authorizeReadWrite, for one-off write actions that don't fit that helper's
// GET/POST/PUT/DELETE shape (e.g. managing a tab's sub-resource). `flag` is
// still honored as a fallback for a role that has it but not the base
// `permission`.
export function requirePermissionOrEditFlag(user, permission, flag, denyMessage) {
  requireAuth(user);
  if (isSuperUser(normalizeRole(user.role))) return;
  if (user.permissions?.includes(permission) || user[flag]) return;
  throw new ApiError(403, denyMessage || "You do not have permission to perform this action.");
}

// Order Processing, Billing, and Dispatch are the only tabs that keep
// requiring a separate edit-flag on top of view access — everywhere else,
// having view access to a tab is enough to do everything in it (add/edit,
// not delete — see authorizeMasterDelete/adminOnlyDelete below).
const PROTECTED_EDIT_PERMISSIONS = new Set(["orders", "billing", "dispatch"]);
...
// user's assigned role, see sanitizeUser). Write access comes from that same
// view permission too, UNLESS `permission` is one of the protected tabs
// above, in which case an explicit edit-flag (`editColumnName`, an
// allow_edit_* key) is still required. `adminOnlyDelete` covers the handful
// of routes where deletion defaults to Admin-only regardless of edit-flag
// (isSuperUser already bypasses everything above, so this only fires for
// non-Admin DELETEs) — `deleteFlag`, an allow_delete_* key, is an optional
// escape hatch so a specific role can still be granted delete rights via
// Manage Roles instead of it being permanently Admin-only with no way to
// delegate it (that gap is exactly what left Returns/Contracts/Godown
// Master/Warranty/FBF-FBA Master delete un-delegable — see
// components/users/constants.js's allow_delete_* entries).
export function authorizeReadWrite(user, method, { permission, editColumnName = null, adminOnlyDelete = false, deleteFlag = null, denyMessage = "You do not have permission to perform this action." }) {
  requireAuth(user);
  if (isSuperUser(normalizeRole(user.role))) return;
  ...
  if (m === "DELETE" && adminOnlyDelete) {
    if (deleteFlag && user[deleteFlag]) return;
    throw new ApiError(403, denyMessage);
  }
  if (permission && !PROTECTED_EDIT_PERMISSIONS.has(permission) && user.permissions?.includes(permission)) return;
...
// masterKey -> the PERMISSIONS_LIST view-permission id that covers it.
// Having that view permission is now enough to add/edit (not delete — see
// authorizeMasterDelete) — mirrors the PROTECTED_EDIT_PERMISSIONS carve-out
// in authorizeReadWrite above, just none of these masterKeys are protected.
const MASTER_KEY_VIEW_PERMISSION = {
  category: "stat_category", brand: "stat_brand", vendor: "stat_vendor", item: "stat_item",
  combo: "stat_combo", unit: "stat_unit", mapping: "stat_mapping",
  roles: "roles", companyMaster: "companyMaster", platformMaster: "platformMaster", deliveryPartnerMaster: "deliveryPartnerMaster",
  users: "users", emailAccounts: "emailAccounts", emailTemplates: "emailTemplates", clientMaster: "clientMaster", carePackMaster: "carePackMaster",
};

// AddProductWizard.jsx (opened from Contract upload/save, to fill in a
// product Item Master doesn't have yet before the contract can link to it)
// creates through these exact same shared /Inventory/* endpoints — never
// edits or deletes an existing brand/category/unit/item/mapping, only ever
// creates new ones. Without this, a Contracts-only role needed separate Add
// rights (or full view access) across five unrelated master tabs just to
// get past the wizard — "give it full masters power" to unblock a
// contracts-only workflow. Scoped to isCreate so this never grants edit/
// delete rights over data that already exists.
//
// This is the single source of truth for "does the wizard's caller get a
// pass here" — app/Inventory/CheckCategoryBrandMapping/route.js (the
// wizard's mapping-exists check, gated by the unrelated authorizeInventory
// instead of authorizeMasterWrite) calls this same export rather than
// re-deciding the same policy inline, so the two can't drift apart.
const CONTRACT_WIZARD_CREATE_MASTERS = new Set(["item", "brand", "category", "unit", "mapping"]);
export const isContractsWizardCreateAccess = (user, masterKey) =>
  CONTRACT_WIZARD_CREATE_MASTERS.has(masterKey) && !!user.permissions?.includes("contracts");

export function authorizeMasterWrite(user, masterKey, { isCreate, denyMessage } = {}) {
  requireAuth(user);
  if (isSuperUser(normalizeRole(user.role))) return;
  const viewPermission = MASTER_KEY_VIEW_PERMISSION[masterKey];
  if (viewPermission && user.permissions?.includes(viewPermission)) return;
  if (isCreate && isContractsWizardCreateAccess(user, masterKey)) return;
...
// Delete deliberately stays stricter than Add/Edit — view access to a tab
// does not imply permanent-delete rights, only the explicit allow_delete_*
// edit-flag does.
