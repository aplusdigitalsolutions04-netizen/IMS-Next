...
// A custom platform field can be type "dropdown" — this holds its option
// list as a JSON array of strings (e.g. '["Prepaid","COD"]'), NULL for every
// other field type. Same lazy self-migration pattern as above.
let fieldOptionsEnsured = false;
export async function ensurePlatformFieldOptionsColumn() {
  if (fieldOptionsEnsured) return;
  try {
    await mysqlPool.query("ALTER TABLE selling_platform_fields ADD COLUMN fieldOptions TEXT NULL");
  } catch (err) {
    if (err.code !== "ER_DUP_FIELDNAME") throw err;
  }
  fieldOptionsEnsured = true;
}
