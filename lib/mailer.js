...
async function ensureSentLogColumn() {
  ...
  // Lets an order's full email history (sent + replies) be looked up by
  // orderGuid — see app/api/orders/[orderGuid]/email-history/route.js.
  // Before this, a sent email had no link back to the order it was about at
  // all (toAddress is just a free-text string, and it can't be trusted:
  // multiple orders can share a buyer/consignee email, and an order's email
  // fields can be edited after the fact).
  try {
    await mysqlPool.query("ALTER TABLE email_sent_log ADD COLUMN orderGuid CHAR(36) NULL");
  } catch (err) {
    if (err.code !== "ER_DUP_FIELDNAME") throw err;
  }
...
export async function sendMail({ companyGuid, purpose = "general", accountGuid, to, cc, bcc, subject, body, bodyHtml, attachments, orderGuid }) {
  ...
  await mysqlPool.query(
    `INSERT INTO email_sent_log (guid, companyGuid, purpose, toAddress, subject, body, messageId, emailAccountGuid, orderGuid)
     VALUES (UUID(), ?, ?, ?, ?, ?, ?, ?, ?)`,
    [companyGuid || null, purpose, to, subject || null, body || (bodyHtml ? stripHtml(bodyHtml) : null), info?.messageId || null, account?.guid || null, orderGuid || null]
  );
...
export async function sendWarrantyEmail({ companyGuid, purpose = "warranty", accountGuid, to, cc, bcc, subject, body, bodyHtml, attachments, orderGuid }) {
  return sendMail({ companyGuid, purpose, accountGuid, to, cc, bcc, subject, body, bodyHtml, attachments, orderGuid });
}
