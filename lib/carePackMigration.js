...
// The Care Pack duration alone (above) never carried a cost — Stock In and
// Item Master's Add Serial both only ever wrote the label. Adding a price
// alongside it lets that cost actually be recorded per serial, e.g. for
// landing-cost accounting or so it can be shown back in the global serial
// search (see components/common/GlobalSearchModal.jsx).
let priceEnsured = false;
export async function ensureCarePackPriceColumn() {
  if (priceEnsured) return;
  try {
    await mysqlPool.query("ALTER TABLE inventorystockinserial ADD COLUMN carePackPrice DECIMAL(10,2) NULL");
  } catch (err) {
    if (err.code !== "ER_DUP_FIELDNAME") throw err;
  }
  priceEnsured = true;
