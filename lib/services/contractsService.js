...
export const contractsService = {
  ...
  deleteContract: async (id, remarks) => {
    const res = await api.delete(`/contracts/${id}`, { data: { remarks } });
    return res.data;
  },
  ...
  // One row per product, contract fields repeated on every row — the exact
  // shape importContracts expects back, so the pair round-trips.
  exportContracts: async () => {
    const res = await api.get("/contracts/export", { responseType: "blob" });
    downloadBlob(res, `contracts_export_${Date.now()}.xlsx`);
    return true;
  },

  // Bulk-creates contracts from an Excel file (same shape exportContracts
  // produces) — bypasses the AI-extraction Upload Contract flow entirely, for
  // bulk data entry/migration. Returns { message, results: { success, failed,
  // skipped, totalRows } }.
  importContracts: async (file) => {
    const formData = new FormData();
    formData.append("file", file);
