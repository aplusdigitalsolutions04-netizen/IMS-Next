...
export const inventoryService = {
  ...
  async getColorTypes() {
    const res = await legacyApi.get(`${API_URL}/GetColorTypeList`);
    return res.data?.data || [];
  },

  async saveColorType(data) {
    const res = await legacyApi.post(`${API_URL}/SaveOrUpdateColorType`, {
      ColorTypeId: data.colorTypeId || "0",
      ColorTypeName: data.colorTypeName,
    });
    return res.data;
  },

  async deleteColorType(colorTypeId) {
    const res = await legacyApi.post(`${API_URL}/DeleteColorType`, { colorTypeId });
    return res.data;
  },

  async getPrinterTypes() {
    const res = await legacyApi.get(`${API_URL}/GetPrinterTypeList`);
    return res.data?.data || [];
  },

  async savePrinterType(data) {
    const res = await legacyApi.post(`${API_URL}/SaveOrUpdatePrinterType`, {
      PrinterTypeId: data.printerTypeId || "0",
      PrinterTypeName: data.printerTypeName,
    });
    return res.data;
  },

  async deletePrinterType(printerTypeId) {
    const res = await legacyApi.post(`${API_URL}/DeletePrinterType`, { printerTypeId });
    return res.data;
  },
