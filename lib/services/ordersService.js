...
export const ordersService = {
  // Reads a PDF/image order document, base64-encodes it, and sends it to the
  // AI extraction endpoint (app/api/ai/parse-file/route.js) which returns the
  // GeM/Amazon/Flipkart order fields as JSON for the New Order form to prefill.
  parseOrderFile: async (file) => {
    ...
    const res = await api.post("/ai/parse-file", { fileBase64, mimeType: file.type });
    return res.data;
  },
  ...
  // Saves a Draft order's model/serial picks WITHOUT confirming it — a
  // picked serial gets reserved (see app/api/orders/draft/[orderId]/reservations/route.js)
  // so it can't be double-picked by another order, but nothing here changes
  // the order's status or creates real dispatched order_items yet.
  saveDraftSelections: async (orderId, items) => {
    const res = await api.post(`/orders/draft/${orderId}/reservations`, { items });
    return res.data;
  },

  getDraftSelections: async (orderId) => {
    const res = await api.get(`/orders/draft/${orderId}/reservations`);
    return res.data?.selections || {};
  },

  sendDraftToBilling: async (orderId) => {
    const res = await api.post(`/orders/draft/${orderId}/send-to-billing`);
    return res.data;
  },
