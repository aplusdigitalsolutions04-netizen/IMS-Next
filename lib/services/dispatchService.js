...
export const dispatchService = {
  getDispatches: async (includeDeleted = true, companyGuid) => {
    try {
      const cg = companyGuid ? `&companyGuid=${encodeURIComponent(companyGuid)}` : "";
      const res = await api.get(`/dispatches?includeDeleted=${includeDeleted}&_t=${new Date().getTime()}${cg}`);
      return res.data;
    } catch (error) {
  ...
  addDispatch: async (data) => {
    ...
    const payload = {
      ...
      modelGuid: data.modelGuid || null,
      itemVariantId: data.itemVariantId || null,
      quantity: data.quantity ? toNumber(data.quantity) : undefined,
      nonSerialized: !!data.nonSerialized,
      ...
      platformFields: data.platformFields || null,
      carePackUpgrade: data.carePackUpgrade || null,
      carePackUpgradePrice: data.carePackUpgradePrice !== undefined && data.carePackUpgradePrice !== "" ? toNumber(data.carePackUpgradePrice) : null,
    };
  ...
  addBulkDispatch: async (items) => {
    const safeItems = items.map((item) => {
      ...
      return {
        ...
        // Mixed tab's non-serialized lines (see components/newDispatch/
        // NewDispatch.jsx) carry these instead of a serialId — dropped here
        // otherwise, since this object literal is an explicit allowlist, not
        // a passthrough of `item`.
        itemVariantId: item.itemVariantId || null,
        quantity: item.quantity !== undefined ? toNumber(item.quantity) : undefined,
        nonSerialized: !!item.nonSerialized,
        carePackUpgrade: item.carePackUpgrade || null,
        carePackUpgradePrice: item.carePackUpgradePrice !== undefined && item.carePackUpgradePrice !== "" ? toNumber(item.carePackUpgradePrice) : null,
        ...
        platformFields: item.platformFields || null,
      };
    });
  ...
  deleteDispatch: async (ids, reason, cancelledBy, clearCharges) => {
