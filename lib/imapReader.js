...
// Lets a received reply be joined straight to the order it was about, via
// the sent email it replied to (which now carries orderGuid — see
// lib/mailer.js's ensureSentLogColumn) — see
// app/api/orders/[orderGuid]/email-history/route.js. Without this, showing
// "here's the reply text" in an order's email history would mean
// re-deriving the In-Reply-To/References match at read time instead of
// once, here, when it's actually known.
let messagesMigrated = false;
async function ensureMessagesColumn() {
  if (messagesMigrated) return;
  try {
    await mysqlPool.query("ALTER TABLE email_messages ADD COLUMN repliedToSentLogGuid CHAR(36) NULL");
  } catch (err) {
    ...
    try {
      ...
      for (const uid of uids || []) {
        ...
        if (existing.length === 0) {
          const newMessageGuid = randomUUID();
          await mysqlPool.query(
            `INSERT INTO email_messages
             ...
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)`,
            [
              newMessageGuid, account.guid, account.companyGuid, account.purpose, messageId,
          ...
          // to cancel the pending reminder for it. Note this isn't scoped to
          // "same address"/"same mailbox" at all — a reply threads by
          // Message-ID regardless of which address or account it actually
          // arrives at, which is exactly what makes an order's email history
          // still pick it up even when the reply doesn't come back from the
          // same address the original was sent to.
          const repliedToIds = [
            parsed.inReplyTo,
            ...(Array.isArray(parsed.references) ? parsed.references : parsed.references ? [parsed.references] : []),
          ].filter(Boolean);
          if (repliedToIds.length > 0) {
            const [matchedSent] = await mysqlPool.query(
              "SELECT guid FROM email_sent_log WHERE messageId IN (?) LIMIT 1",
              [repliedToIds]
            );
            await mysqlPool.query(
            ...
            if (matchedSent.length > 0) {
              await mysqlPool.query(
                "UPDATE email_messages SET repliedToSentLogGuid = ? WHERE guid = ?",
                [matchedSent[0].guid, newMessageGuid]
              );
            }
          }
        }
