...
export async function updateDispatchItem(pool, itemId, fields, user, companyGuid) {
  ...
  try {
    ...
    await conn.query(
      "UPDATE order_items SET sellingPrice = ?, warranty = ?, contractFilename = ? WHERE guid = ? AND companyGuid = ?",
      [merged.sellingPrice ?? current.sellingPrice, merged.warranty ?? current.warranty, merged.contractFilename ?? current.contractFilename, itemId, companyGuid]
    );
