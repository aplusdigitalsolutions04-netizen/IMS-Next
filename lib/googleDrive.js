...
//
// The refresh token itself lives in the DB (google_drive_auth), not
// GOOGLE_OAUTH_REFRESH_TOKEN / .env.local — the callback route used to write
// it straight into .env.local on disk and ask for a process restart, which
// only ever worked for local dev. On the real deployment (Passenger shared
// hosting) that file often isn't writable by the app process, and even when
// it is, there's no way to trigger the restart the change needs — so
// authorizing on the live site silently never took effect. A DB row takes
// effect on the very next request, no restart involved, on any host.
...
// 30s cache, same tradeoff as the user/company-active caches elsewhere in
// this codebase — avoids a DB round trip on every single Drive call.
...
async function getStoredRefreshToken() {
  ...
  const [rows] = await mysqlPool.query("SELECT refreshToken FROM google_drive_auth WHERE id = 1");
  // Falls back to the env var only for whatever's already been set up that
  // way on an existing deployment — saveRefreshToken() below is what every
  // *new* authorization goes through from here on.
  const value = rows[0]?.refreshToken || process.env.GOOGLE_OAUTH_REFRESH_TOKEN || null;
