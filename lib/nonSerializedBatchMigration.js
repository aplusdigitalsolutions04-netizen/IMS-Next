...
// Added after the table itself — a batch added directly from Item Master
// (see app/Inventory/AddVariantStock/route.js) can record which vendor it
// was bought from, same as a serialized serial's own vendorId.
let vendorColumnEnsured = false;
export async function ensureNonSerializedBatchVendorColumn() {
  if (vendorColumnEnsured) return;
  await ensureNonSerializedBatchTable();
  try {
    await mysqlPool.query("ALTER TABLE inventorynonserializedbatch ADD COLUMN vendorId CHAR(36) NULL");
  } catch (err) {
    if (err.code !== "ER_DUP_FIELDNAME") throw err;
  }
  vendorColumnEnsured = true;
}
