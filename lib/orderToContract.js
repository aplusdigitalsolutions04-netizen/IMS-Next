...
export async function saveOrderAsContract(orderGuid, user, { replace = false } = {}) {
  ...
  if (dup.length > 0) {
    ...
    // Replace, not the Contracts-tab delete flow (app/api/contracts/[id]/route.js
    // DELETE) — that also cancels the linked order, which is wrong here since
    // the order is the source of truth being re-saved, not something to undo.
    await mysqlPool.query(
      "UPDATE contracts SET isDeleted=1, modifiedBy=?, modifiedAt=NOW() WHERE guid=? AND companyGuid=?",
      [user.username || user.fullName || "Unknown", dup[0].guid, user.companyId]
    );
  }
